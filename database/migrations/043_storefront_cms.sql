...
CREATE TABLE IF NOT EXISTS legal_policies (
    ...
    updated_at      TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    UNIQUE KEY uq_legal_policies_slug (slug),
    KEY idx_legal_policies_pub (is_published, show_in_footer, sort_order)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Hero banners: add via Admin → Hero banners (no demo slides on fresh install).
