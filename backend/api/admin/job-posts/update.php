...
$check = $pdo->prepare('SELECT id FROM job_posts WHERE id = ?');
$check->execute([$id]);
...
try {
    ...
    $pdo->prepare(
        'UPDATE job_posts SET title = ?, city = ?, job_type = ?, description = ?, is_active = ? WHERE id = ?'
    )->execute([
        $title,
        $city,
        $role['slug'],
    ...
    Response::error('Could not update job post.', 500);
}
